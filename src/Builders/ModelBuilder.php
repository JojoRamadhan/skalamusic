<?php
namespace Jojoramadhan\Skalamusic\Builders;

use Illuminate\Support\Str;
use Jojoramadhan\Skalamusic\Cache\QuickCache;
...
class ModelBuilder
{
    ...
    */
    ...
    {
        if($input != \'\') {
            $this->attributes[\'' . $field->title . '\'] = Carbon::createFromFormat(config(\'skalamusic.date_format\'), $input)->format(\'Y-m-d\');
        }else{
    ...
    {
        if($input != \'0000-00-00\') {
            return Carbon::createFromFormat(\'Y-m-d\', $input)->format(config(\'skalamusic.date_format\'));
        }else{
            return \'\';
        }
    }' . "\r\n\r\n";
    ...
    {
        if($input != \'\') {
            $this->attributes[\'' . $field->title . '\'] = Carbon::createFromFormat(config(\'skalamusic.date_format\') . \' \' . config(\'skalamusic.time_format\'), $input)->format(\'Y-m-d H:i:s\');
        }else{
            $this->attributes[\'' . $field->title . '\'] = \'\';
        }
    }
    ...
    {
        if($input != \'0000-00-00\') {
            return Carbon::createFromFormat(\'Y-m-d H:i:s\', $input)->format(config(\'skalamusic.date_format\') . \' \' .config(\'skalamusic.time_format\'));
        }else{
            return \'\';
        }
    }' . "\r\n\r\n";
